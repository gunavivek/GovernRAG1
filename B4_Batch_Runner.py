...
RUN from repo root:
  python B4_Batch_Runner.py --records "data\delucionqa_records_912.csv" --k 25
  python B4_Batch_Runner.py --records "data\delucionqa_records_912.csv" --k 25 --run-e3 --judge-model gemini-2.5-pro
  python B4_Batch_Runner.py ... --batches 0 1        # only specific batch indices (smoke test)
...
RES = ROOT / "results"; FULL = RES / "full_run"; FULL.mkdir(parents=True, exist_ok=True)
...
TIMING_LOG = FULL / "timing_log.csv"   # execution log: batch,phase,config,seconds,n_records,ok,when
...
def load_state(records, k):
    if STATE.exists():
        s = json.load(open(STATE, encoding="utf-8"))
        if s.get("records") == records and s.get("k") == k:
            return s
        print("[B4][WARN] state.json is for records=%s k=%s -- starting FRESH state (old file kept as state.bak.json)"
              % (s.get("records"), s.get("k")))
        shutil.copy(STATE, FULL / "state.bak.json")
    return {"records": records, "k": k, "done": []}
...
def main():
    ...
    nb = (total + k - 1) // k
    state = load_state(os.path.abspath(args.records), k)
    ...
    # 5) merge accumulators into the canonical paths E3 reads
    print("\n[B4] all %d batches complete -- merging into results/ for E3" % nb)
    ...
    # 6) score
    e3 = [sys.executable, "-u", str(ROOT / "E3_Unified_Evaluation.py"), "--judge-model", args.judge_model]
    if args.run_e3:
        print("[B4] running E3 ...")
        if not run(e3):
            sys.exit("[B4][FATAL] E3 failed -- caches are merged, rerun E3 alone.")
    else:
        print("[B4] NEXT: %s" % " ".join(e3))
